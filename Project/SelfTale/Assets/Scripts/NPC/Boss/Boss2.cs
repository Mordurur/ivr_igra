...
public class Boss2 : EnemyBase
{
    ...
    {
        if (transform.position.x - playerChar.transform.position.x > 0 && facingRight && canWalk)
        {
            Flip();
        }
        else if (transform.position.x - playerChar.transform.position.x <= 0 && !facingRight && canWalk)
        {
            Flip();
        }
        velocity.x += kback;
    ...
    public override IEnumerator FlipWait()
    {
        canWalk = false;
